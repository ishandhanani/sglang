...
class TestRouterKVReuse(unittest.TestCase):
    ...
    def test_remote_g2_direct_fetch_worker_backpressure_skips_hbm_allocation(self):
        ...
        handler.peer_endpoints = {"7:0": "http://127.0.0.1:39007"}
        handler.direct_transfer = SimpleNamespace(enabled=True)
    ...
    def test_g2plus_transfer_parallelism_prefers_backend_neutral_env(self):
        ...
        self.assertEqual(ServerArgs.g2plus_transfer_parallelism, 4)
    ...
    def test_server_args_cli_wires_g2plus_flags(self):
        ...
        self.assertTrue(server_args.enable_g2plus)
        self.assertTrue(server_args.enable_router_kv_reuse)
        self.assertTrue(server_args.enable_hierarchical_cache)
        self.assertEqual(server_args.g2plus_worker_id, 7)
        self.assertEqual(server_args.g2plus_endpoint, "127.0.0.1:39007")
        self.assertEqual(
            server_args.g2plus_peer_endpoints, '{"8":"127.0.0.1:39008"}'
        )
        self.assertEqual(server_args.g2plus_fetch_workers, 3)
        self.assertEqual(server_args.g2plus_transfer_parallelism, 4)
        self.assertEqual(server_args.g2plus_timeout_secs, 2.5)
        self.assertEqual(server_args.g2plus_transfer_backend, "nixl")

    def test_server_args_rejects_g2plus_without_worker_id(self):
        parser = argparse.ArgumentParser()
        ServerArgs.add_cli_args(parser)

        args = parser.parse_args(["--model-path", "dummy", "--enable-g2plus"])

        with self.assertRaisesRegex(
            ValueError, "--enable-g2plus requires --g2plus-worker-id"
        ):
    ...
    def test_server_args_allows_generic_router_reuse_without_g2plus_worker_id(self):
        ...
        args = parser.parse_args(
            ["--model-path", "dummy", "--enable-router-kv-reuse"]
        )
        server_args = ServerArgs.from_cli_args(args)

        self.assertTrue(server_args.enable_router_kv_reuse)
        self.assertFalse(server_args.enable_g2plus)
        self.assertIsNone(server_args.g2plus_worker_id)

    def test_server_args_rejects_invalid_g2plus_peer_endpoint_values(self):
        parser = argparse.ArgumentParser()
        ServerArgs.add_cli_args(parser)
        ...
        with self.assertRaisesRegex(
            ValueError, "--g2plus-peer-endpoints values must be non-empty strings"
        ):
            ServerArgs.from_cli_args(args)

    def test_server_args_rejects_invalid_g2plus_endpoint_values(self):
        parser = argparse.ArgumentParser()
        ServerArgs.add_cli_args(parser)
        ...
        with self.assertRaisesRegex(
            ValueError, "--g2plus-endpoint values must be non-empty strings"
        ):
            ServerArgs.from_cli_args(args)
    ...
    def test_remote_g2_handler_falls_back_to_all_peer_endpoints(self):
        handler = RemoteG2ReuseHandler.__new__(RemoteG2ReuseHandler)
        handler.worker_id = None
        handler.dp_rank = 0
        handler.endpoint = None
        handler.peer_endpoints = {
