...
def _select_dp_endpoint(endpoint_spec: Optional[str], dp_rank: int) -> Optional[str]:
...
def _parse_peer_endpoints(peer_endpoints: Optional[str]) -> Dict[str, str]:
...
class RemoteG2ReuseHandler:
    def __init__(
        ...
        *,
        ...
        self.timeout_secs = server_args.g2plus_timeout_secs
        self.direct_transfer = direct_transfer
        self.metrics_collector = metrics_collector
        self.allow_http_staging = (
            str(getattr(server_args, "g2plus_transfer_backend", "auto")).lower()
            == "http"
        )
        ...
        self.endpoint = _select_dp_endpoint(server_args.g2plus_endpoint, dp_rank)
        self.peer_endpoints = _parse_peer_endpoints(server_args.g2plus_peer_endpoints)
        self._source_server: Optional[ThreadingHTTPServer] = None
        self._source_thread: Optional[threading.Thread] = None
        self._shutdown = False
        worker_limit = max(1, int(server_args.g2plus_fetch_workers))
    ...
    def from_scheduler(cls, scheduler) -> Optional["RemoteG2ReuseHandler"]:
        ...
        if not hasattr(scheduler.tree_cache, "_insert_helper_host"):
            ...
            return None
        if getattr(server_args, "g2plus_worker_id", None) is None:
            logger.warning(
                "Router KV reuse disabled because G2plus worker identity is not set; "
                "set --g2plus-worker-id when enabling G2plus remote KV reuse"
            )
            return None
        ...
        return cls(
            ...
            worker_id=server_args.g2plus_worker_id,
    ...
    def _candidate_endpoints_for_plan(self, plan: RemoteKvReusePlan) -> list[str]:
        ...
        endpoint = self.peer_endpoints.get(
            f"{plan.source_worker_id}:{plan.source_dp_rank}"
        )
        add(endpoint)
        add(self.peer_endpoints.get(str(plan.source_worker_id)))
        if (
            ...
            add(self.endpoint)
        for endpoint in self.peer_endpoints.values():
            add(endpoint)
        return endpoints
