...
class PerformanceTestParams:
    ...
    spec_num_accept_tokens_threshold: Optional[float] = None
...
def run_performance_test(
    ...
    if spec_num_accept_tokens_threshold is not None:
        print(f"  Spec accept length threshold: {spec_num_accept_tokens_threshold}")
    print(f"{'='*60}\n")
        ...
        if success and results:
            ...
            if spec_num_accept_tokens_threshold is not None:
                if avg_spec_accept_length is None:
                    ...
                    passed = False
                    print(f"✗ {error_msg}")
                elif avg_spec_accept_length < spec_num_accept_tokens_threshold:
                    error_msg = (
                        f"Spec accept length {avg_spec_accept_length:.2f} < "
                        f"threshold {spec_num_accept_tokens_threshold}"
                    )
                    passed = False
                    print(f"✗ {error_msg}")
                else:
